...
void mousePressed() {//activated when a mouse is pressed
  for(int i=0;i<players.size();i++){
    if(players.get(i).Qthisplayer()){
      if (mouseButton == LEFT) {
        terrain.spawnuniter(mouseX-locationX,mouseY-locationY,players.get(i).team,1,selectedunit);//uniter passif
        // Unit(float Xdonne,float Ydonne,int team,int combatdonne,String nomuniterdonne){//data
      }
    }
  }
}
